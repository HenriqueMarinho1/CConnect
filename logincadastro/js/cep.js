...
function geocodeCEP(cep) {
    const apiKey = '764f65b7bdc146e0a221a39ca1fc250c';
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (data.results && data.results.length > 0) {
                ...
                document.getElementById('result').innerHTML = `<p>Não foi possível obter as coordenadas para o CEP informado.</p>`;
            }
        })
        .catch(error => {
            console.error('Erro ao obter as coordenadas:', error);
            console.log('Detalhes do Erro:', error.message);
            document.getElementById('result').innerHTML = `<p>Ocorreu um erro ao obter as coordenadas: ${error.message}</p>`;
        });
}
